  ...
  def solve_brute(board, [x, y] \\ [-1, 0]) do
    if full?(board) do
      ...
      if correct?(board) do
        ...
        {:cont, [x, y]} = next_empty_elem(board, [x, y])

           solve_brute(put(board, [x, y], 1), [x, y])
        || solve_brute(put(board, [x, y], 2), [x, y])
        || solve_brute(put(board, [x, y], 3), [x, y])
        || solve_brute(put(board, [x, y], 4), [x, y])
        || solve_brute(put(board, [x, y], 5), [x, y])
        || solve_brute(put(board, [x, y], 6), [x, y])
        || solve_brute(put(board, [x, y], 7), [x, y])
        || solve_brute(put(board, [x, y], 8), [x, y])
        || solve_brute(put(board, [x, y], 9), [x, y])
      else
        false
      end
    end
